...
export const ProductsView: React.FC<ProductsViewProps> = ({ token }) => {
  ...
  const handleDeleteProduct = async (prodId: string) => {
    if (!window.confirm("Apakah Anda yakin ingin menghapus produk ini? Semua paket sewa terkait juga akan terhapus.")) return;
    try {
  ...
  const handleDeletePackage = async (pkgId: string) => {
    ...
    if (!window.confirm("Hapus paket pilihan?")) return;
    try {
