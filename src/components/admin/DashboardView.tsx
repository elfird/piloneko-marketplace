...
import { ShoppingCart, DollarSign, Database, Award, ClipboardList, ChevronRight, AlertTriangle } from "lucide-react";
  ...
  if (error || !stats) {
    return (
      <div className="p-8 text-center max-w-sm mx-auto">
        ...
        <button onClick={fetchStats} className="px-4 py-2 bg-cyber-surface border border-accent-primary text-white text-xs">RETRY</button>
      </div>
    );
  }
    ...
    <div className="space-y-8 text-left font-sans">
      ...
      <div>
        ...
        <p className="text-[10px] text-cyber-muted font-mono uppercase tracking-widest block">
          Tinjauan statistik transaksi penjualan instan dan kontrol stok sirkulasi
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {cards.map((c) => (
          ...
          >
            ...
            <div>
              <span className="text-[10px] font-orbitron tracking-widest font-bold opacity-80 block truncate">
                {c.title}
              </span>
              <span className="text-xl sm:text-2xl font-orbitron font-black block mt-2 text-white">
                {c.value}
              </span>
              <span className="text-[9px] font-mono tracking-widest block mt-2 font-semibold uppercase opacity-65">
                // {c.desc}
              </span>
            </div>

            <div className="p-2 border border-current bg-[#000]/15 shrink-0 rounded-xs">
              {c.icon}
            </div>
          </div>
        ))}
      ...
      {/* Warning block for items low in stocks */}
          ...
          <div className="flex gap-3 items-start">
            ...
            <div>
              ...
              <p className="text-xs text-cyber-muted font-sans mt-0.5">
                Tedapat {stats.lowStockWarnings.length} Paket sewa akun premium dengan persediaan kurang dari 4 stok. Segera isi ulang!
              </p>
            </div>
          </div>
      ...
      {/* Low list panel showing bottom tables */}
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-accent-primary/10 text-cyber-muted font-orbitron uppercase tracking-widest text-[9px]">
                  ...
                  <th className="py-3 px-4 font-bold">PAKET / DETAIL</th>
                ...
                {stats.recentOrders && stats.recentOrders.length > 0 ? (
                  stats.recentOrders.map((ord: any) => (
                    <tr key={ord.id} className="hover:bg-cyber-surface/30 transition-colors">
                      <td className="py-3.5 px-4 font-mono font-bold text-accent-secondary select-all">
                        {ord.refCode}
                      </td>
                      ...
                      <td className="py-3.5 px-4 font-semibold text-accent-primary">
                        {ord.productName}
                      </td>
                      <td className="py-3.5 px-4 text-cyber-text truncate max-w-xs">
                        {ord.packageName}
                      </td>
                      <td className="py-3.5 px-4 font-orbitron font-bold text-white whitespace-nowrap">
                        {formatPrice(ord.price)}
                      </td>
                      <td className="py-3.5 px-4">
                        <span
                          className={`px-2 py-0.5 font-mono text-[9px] uppercase border inline-block rounded-xs font-bold ${
                            ord.status === "PENDING"
