...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  // Stock status
  const totalStock = activePackages.reduce((acc, p) => acc + (p.stockCount || 0), 0);
  const isGameTopUp = product.category?.type === "GAME_TOPUP" || product.slug.includes("game") || product.category?.name.includes("Game");

  const isOutOfStock = !isGameTopUp && totalStock <= 0 && activePackages.length > 0;
        ...
        <div className="border-t border-accent-primary/10 pt-3 flex items-end justify-between mt-auto">
          ...
          <div className="text-right">
            {isGameTopUp ? (
              ...
              <span className="text-[10px] bg-red-900/40 text-red-400 px-1.5 py-0.5 font-mono border border-red-500/40 rounded-xs uppercase">
                OUT OF STOCK
              </span>
            ) : (
